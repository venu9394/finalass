...
public class PayData_Temp_29_01_2018_80D extends HttpServlet {
	...
	*/
	...
	//String FromPeriod="201609";
	   ...
	   int currentPayperiod=201803; // need to change current payperiod every time every month
    ...
    Pt.append(" group by b.employeesequenceno; ");*/
		...
		rs=null;
		try {
			rs=(ResultSet)DataObj.getBasic("SELECT EMPLOYEEID,S80D,D80D,CTC FROM test_mum.tbl_emp_tds_extra_80d_cal where EMPLOYEEID in (select employeesequenceno from hclhrm_prod.tbl_employee_primary where employeeid in ("+EmployeeIDs_Bulk+"))", "80OUTER CALICULATIONS", rs ,connection);
			while(rs.next()){
...
/*	87 /arrear
		...
		try {
			...
			rs=(ResultSet)DataObj.getBasic("Select abs(TIMESTAMPDIFF(MONTH, date((concat("+MaxPayPeriod+",25))),date((concat("+ToPeriod+",25)))))+1  from dual", "ProposedPeriod", rs ,connection);
			  ...
			  if(Integer.parseInt(MaxPayPeriod)< 201712){
