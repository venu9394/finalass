...
public class NewJoinees extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		java.sql.PreparedStatement ps=null;
		 ...
		 LOGGER.fatal("New Joinee Servlet invoked....!");
	...
	if(Routing.equalsIgnoreCase("MyProfile")){
		...
		*/
		...
		Emp_DOJ.append(" SELECT A.REQ_DATE,A.FROM_DATE,A.TO_DATE,concat(A.MESSAGE,' & Subject:',A.SUBJECT) MESSAGE ,  ");
    ...
    // ************** For Leave Balance Caliculation *****************************/////////////////////////
    	...
    	Leave_Quota.append(" select C.employeesequenceno,trim(A.SHORTNAME) SHORTNAME,B.quantity from  hclhrm_prod.tbl_leave_type A,  ");
    	...
    	Leave_Quota.append(" B.Leavetypeid=A.Leavetypeid and B.year=year(now())");
			...
			try {
				...
				while(Res.next()){
					...
					if(Routing.equalsIgnoreCase("NewJoine")){	
						...
						/*LeaveBalance.put("SL_REM" , Integer.parseInt(LeaveBalance.get("SL").toString()));
						LeaveBalance.put("EL_REM" , Integer.parseInt(LeaveBalance.get("EL").toString()));
						LeaveBalance.put("CL_REM" , Integer.parseInt(LeaveBalance.get("CL").toString()));*/
						...
						if(Res.getString("LEAVE_TYPE").equalsIgnoreCase("CL")){
							...
							if(Res.getString("FLAG").equalsIgnoreCase("P")){
								...
								//LeaveBalance.put("CL_TOT" , LeaveBalance.get("CL"));
							   CL=Integer.parseInt(Res.getString("Leaves"));
								//LeaveBalance.put("CL_REM" , Integer.parseInt(LeaveBalance.get("CL").toString())- Integer.parseInt(Res.getString("Leaves")));
							...
							if(Res.getString("FLAG").equalsIgnoreCase("P")){
								...
								SL=Integer.parseInt(Res.getString("Leaves"));
							//	LeaveBalance.put("SL_REM" , Integer.parseInt(LeaveBalance.get("SL").toString())- Integer.parseInt(Res.getString("Leaves")));
							...
							if(Res.getString("FLAG").equalsIgnoreCase("P")){
								...
								EL=Integer.parseInt(Res.getString("Leaves"));
								//LeaveBalance.put("EL_REM" , Integer.parseInt(LeaveBalance.get("EL").toString())- Integer.parseInt(Res.getString("Leaves")));
				...
				try{
					...
					LeaveBalance.put("SL_TOT" , LeaveBalance.get("SL"));
					LeaveBalance.put("EL_TOT" , LeaveBalance.get("EL"));
					LeaveBalance.put("CL_TOT" , LeaveBalance.get("CL"));
					
				LeaveBalance.put("SL_REM" , (Integer.parseInt(LeaveBalance.get("SL").toString())-SL));
				LeaveBalance.put("EL_REM" , (Integer.parseInt(LeaveBalance.get("EL").toString())-EL));
				LeaveBalance.put("CL_REM" , (Integer.parseInt(LeaveBalance.get("CL").toString())-CL));
				}catch(Exception EDP){
					
					System.out.println("Exception At EDP:: "+ EDP);
