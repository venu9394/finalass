...
System.out.println("Calling ASSAM V2.0 @JSP");
...
try{
	...
	if(Month_View=="Payperiod"){
		...
		//alert(Month_View);
		 ...
		 $http({
		        method : "POST",
		        url : "Attendance_assam2?Routing=ATTENDANCE_BIO_DATES&ATT_FLAG=DATES&Month="+Month_Sel+"",
		...
		$http({
	        method : "POST",
	        url : "Attendance_assam2?Routing=ATTENDANCE_BIO_DATES&ATT_FLAG=MONTHS&Month="+Month_Sel+"",
		...
		if(val=="My_Datas"){
			...
			var Month_Sel="";
			 ...
			 $http({
			        method : "POST",
			        url : "Attendance_assam2?Routing=ATTENDANCE_BIO_DATES&ATT_FLAG=DATES&Month_FROM="+Month_FROM+"&Month_TO="+Month_TO+" ",
...
$scope.newICTReq=function(event){
	
	//alert("Mahesh Angular Calling"+event.target.name);
	...
	document.getElementById("tags2").value="";
	 ...
	 document.getElementById("Requ_Date").innerHTML=event.target.id;
	 document.getElementById("Responce_Message").innerHTML='';
	 document.getElementById("Responce_Message_btn").style.display='';
	 
	 document.getElementById("Requ_Date_Temp").value=event.target.name; 
	
	 
	 $http({
    	    url: "Attendance_assam2",
    	    method: "POST",
    	    params: {
    	    	Routing :"ReqSpenthours",
    	    	onDate:event.target.id
    	    },
    	}).then(function mySucces(response) {
	 ...
	 $scope.NewAttendanceRequest=function(event){
		 
		 //reqCodeNum;
		//alert(event.target.id);
		 document.getElementById("Responce_Message").innerHTML="";
		$scope.test =event.target.id;
			
	//	alert($scope.test.split("#")[1].length);
			 ...
			 var toemail=document.getElementById("tags2").value;
			    ...
			    if(toemail.length <4){
			    ...
			    } else if($scope.test.split("#")[1].length<6){
				   ...
				   $http({
			    	    ...
			    	    method: "POST",
			    	    params: {
			    	    	Routing :"ReqSpenthoursInsert",
			    	    	snoCode:$scope.test.split("#")[0],
			    	    	reqMsg:$scope.test.split("#")[1],
			    	    	ccEmail:ccemail,
			    	    	toEmail:toemail,
			    	    	newSubject:Subject
			    	    },
			    		...
			    		document.getElementById("Responce_Message").innerHTML=data;
												...
												<table class="table table-striped ">
													...
													<tr ng-repeat="x in Data_1">
														...
														<td class="text-center">{{ x.LESSHOURS }}</td>
														  <td align="center";>  <input type='button' class="btn btn-primary btn-sm"  name="{{ x.INNER }}" id="{{ x.DATE }}" data-toggle="modal" data-target="#myModal" id="myBtn" ng-click="newICTReq($event);" value='Request' style='display:{{ x.DAF }}'  >   </td> 
					...
					<div class="vertical-alignment-helper">
					  <div class="modal-dialog vertical-align-center" role="document">
						...
						<div class="modal-content">
						  ...
						  <div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" data-keyboard="false" saria-hidden="true">
					<div class="vertical-alignment-helper">
					  <div class="modal-dialog vertical-align-center modal-lg" role="document">
						<div class="modal-content">
						  <div class="modal-header">
						 <!--  <img src="HR/round-close-button-png-1.png" class="close" height="40" width="40" aria-label="Close"  ng-click="myFunction('My')" data-dismiss="modal"> -->
							<!-- <img src="HR/round-close-button-png-1.png" class="close" height="40" width="40" aria-label="Close"  ng-click="myFunction('My')" data-dismiss="modal"> -->
							...
							<div class="modal-body">
								<form action="ictrequestmail" method="post" >
									...
									<div class="form-group">
									  <label class="col-md-4 control-label">To-Mail</label>
									   <div class="col-sm-8">
									     <input type="email" class="form-control addresspicker input-sm" name="toemail"  id="tags2" value="" placeholder="someone@example.com" >
									  </div>
									</div>
									...
									<div class="form-group">
									  <label class="col-md-4 control-label">CC-Mail</label>
									   <div class="col-sm-8">
									     <input type="email" class="form-control addresspicker input-sm" name="ccemail"  id="tags3" value="" placeholder="someone@example.com" >
									  </div>
									</div>
							...
							<div id="yesdata" style="display:'' ">
									...
									<table class="table table-striped table-condensed table-sm" >
										...
										<tbody>
											<tr>
											<tr data-ng-repeat="M in DataM" >
														<td class="text-center">{{ M.OUT }}</td>
														<td class="text-center">{{ M.IN }}</td>
														<td class="text-center">{{ M.NET }}</td>
												
												<td><textarea class="form-control input-sm" rows="2" id="message" name="message"  ng-model="NewMsg"></textarea></td>
											
												
												<td><input type="button" ng-disabled="M.btnFlag=='P'|| M.btnFlag=='A' || M.btnFlag=='R' "  style="display:" class="btn btn-primary btn-sm" data-ng-model="reqCodeNum" id="{{ M.ReqCode }}# {{NewMsg}}"   data-ng-click="NewAttendanceRequest($event)" value=" {{ M.btnValue }} " /></td>
										...
										<div class="form-group"><span id="Responce_Message" style="color:red;" class="align-right"> </span></div>
