...
<script>
 ...
 function ICT_Req(data){
	...
	/*  document.getElementById("date").value=data.id; 
	  ...
	  */
	  var promptsdd="NO_DATA";
	var DataVal=data.value;
	if(data.value!="Close.File"){
		
	   promptsdd=window.prompt("Comment/Feed Back");
	  ...
	  if(promptsdd!=null){
	  AttendanceRequest_Month_VEN(data.alt,data.name,DataVal,data.id,promptsdd);
	  }else{
 ...
 function AttendanceRequest_Month_VEN(val1,val2,val3,val4,promptsdd){
	  ...
	  //alert(val1 + "~"  + val2 + "~" + val3 );
		...
		var formData = {id:""+val1+"",rid:""+val2+"",flag:""+val3+"",APP_MOD:"MG",comment:""+promptsdd+""};
	try{
	    $.ajax({
	          type: "post",
	          url: "ReqAcpt_Resi",
...
<body  ng-app="myApp" ng-controller="formCtrl"  onload="disableBackButton(); noBack();">
			...
			<div class="inner-wrapper">
								...
								<div class="panel panel-primary">
								   ...
								   <span id='Requ_Date_ERROR' style='color:red;' align='right'>   </span>
									...
									<div class="panel-body">
													...
													<thead class="thead-default " style="background-color:#f2f2f2; color:#777777;">
														<tr>
														   ...
														   <th class="text-center">Feed Back</th>
															 ...
															 <th class="text-center">Action By(HR)</th>
										...
										<td class="text-center"><a href="#"  class="red-tooltip"  data-toggle="tooltip" data-placement="bottom" title='{{ x.REASON }} '><span class="glyphicon glyphicon-envelope"></span>  REASON</a>	</td>		
				...
				<script>
					...
					$('[data-toggle="tooltip"]').tooltip();   
													...
													<td class="text-center"> <a href="#" class="red-tooltip"  data-toggle="tooltip" data-placement="bottom" title='{{ x.HRS_COMMENTS }}' ><span class="glyphicon glyphicon-envelope"></span>{{ x.CC_STATUS }} </a> </td>
														
													<!--  <td class="text-center">    {{ x.CC_STATUS }}  </a> -->
    ...
    <li><a href="#" data-toggle="tooltip" data-placement="right" title="Hooray!">Right</a></li>
														...
														<!--  <td class="text-center"> <span  id="{{ x.RID }}_ST" > {{ x.Manager_Status }}  </span> </td> -->
														 
														 <td class="text-center" colspan='2'> <input type='button' class='btn btn-primary btn-sm'  name='{{ x.ID }}' alt='{{ x.ID }}' id='{{ x.ID }}_1'   onclick='ICT_Req(this);' value='Approve' style='display:{{x.A}}'  > &nbsp; <input type='button' class='btn btn-primary btn-sm'  alt='{{ x.ID }}' name='{{ x.ID }}'  id='{{ x.ID }}_2'   onclick='ICT_Req(this);' value='Reject' style='display:{{x.R}}'  /> &nbsp; <input type='button' class='btn btn-primary btn-sm'  alt='{{ x.ID }}' name='{{ x.ID }}'  id='{{ x.ID }}_2'   onclick='ICT_Req(this);' value='Close.File' style='display:{{x.C}}'  /> </td> 
