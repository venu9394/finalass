...
<table class="table" id="tblData" border='1'>
 ...
 <tr> <th> Emp ID</th>  <th>Name</th>  <th>Basic</th> <th>CA</th> <th>HRA</th> <th>Medical</th> <th>Education</th> <th>PT(Dedu)</th> <th>Gross</th> <th>Pay Period</th> <th>Months</th>
 ...
 <td id="<%=empid+"pg" %>" > <b> <%=FinalComponents.get(empid+"-totalgros") %> </b> </td>
 
		  
  <td id="<%=empid+"pg" %>" >  <%=TaxException.get(empid+".houserent") %>  		</td>
  <td id="<%=empid+"pg" %>" >  <%=TaxException.get(empid+".HouseFinal") %>  	</td>
  <td id="<%=empid+"pg" %>" >  <%=TaxException.get(empid+".Basi40") %>  		</td>
  <td id="<%=empid+"pg" %>" >  <%=TaxException.get(empid+"_CA_M") %>  			</td>
  <td id="<%=empid+"pg" %>" >  <%=TaxException.get(empid+".Medical") %>  		</td>
  <td id="<%=empid+"pg" %>" >  <%=TaxException.get(empid+".ChildEdu") %>  		</td>
 
  <td id="<%=empid+"pg" %>" >  <%=TaxException.get(empid+".ExmpAmount") %>  			</td>
 ...
 <!-- <td id="<%=empid+"pg" %>" >  <%=TaxSections.get(empid+"80D").toString() %>  	</td>  -->
   ...
   <td id="<%=empid+"pg" %>" >  <%=TaxSections.get(empid+"TxamT").toString()%>  </td> 
   
   <td id="<%=empid+"pg" %>" >  <%=TaxSections.get(empid+"_EmpTax_E").toString()%>  </td>
   <td id="<%=empid+"pg" %>" >  <%=TaxSections.get(empid+"_EmpTaxPer_E").toString()%>  </td>
   <td id="<%=empid+"pg" %>" >  <%=TaxSections.get(empid+"_EmpTaxAddl_E").toString()%>  </td>
   
   <td id="<%=empid+"pg" %>" >  <%=TaxSections.get(empid+"_Emp_Tax_E").toString()%>  </td>
   
   
  <td id="<%=empid+"pg" %>" >  <%=TaxSections.get(empid+"Educess").toString() %></td> 
 
  <td style="color:Red" id="<%=empid+"pg" %>" >  <%=TaxSections.get(empid+"tx_Paid").toString() %>  </td> 
  ...
  <td style="color:Red" id="<%=empid+"pg" %>" >  <%=TaxSections.get(empid+"_TDS_F").toString() %>  </td> 
   ...
   <td>      <%=ProposedMonths %>  </td>
    ...
    <td style="color:Red" id="<%=empid+"pg" %>" >  <%=TaxSections.get(empid+"tx_Paid_F").toString() %>  </td> 
    
     <td style="color:Red" id="<%=empid+"pg" %>" >  <%=TaxSections.get(empid+"Paid_Month").toString() %>  </td> 
     
    
  <!--  <td style="color:Green" id="<%=empid+"pg" %>" >  <%=Math.round(Double.parseDouble(TaxSections.get(empid+"tx_Paid").toString())/Total_Months) %>  </td>   -->  
