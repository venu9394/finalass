...
<% 
  ...
  String Login_bu_jason=(String)request.getAttribute("Login_bu_jason");
  String CompanySelected=(String)request.getAttribute("CompanySelected");
...
app.controller('formCtrl', function($scope,$http) {
   ...
   $scope.Login_bu_jason=<%=Login_bu_jason%>;
...
try{
	 $scope.myFunction = function(val) {
		...
		var BuUnit=$('#Fill_Bu').val();
		
			var PAYPERIOD="MONTH";
			if ($('#ATT_PAYPERIOD_HR').is(":checked"))
			{
			  // it is checked
				PAYPERIOD="PAYPERIOD";
			}
			
			//alert("PAYPERIOD::"+PAYPERIOD);
		...
		// $scope.Data_2={"DAY1":"DAY1","DAY2":"DAY2","DAY3":"DAY3","DAY4":"DAY4","DAY5":"DAY5","DAY6":"DAY6","DAY7":"DAY7","DAY8":"DAY8","DAY9":"DAY9","DAY10":"DAY10","DAY11":"DAY11","DAY12":"DAY12","DAY13":"DAY13","DAY14":"DAY14","DAY15":"DAY15","DAY16":"DAY16","DAY17":"DAY17","DAY18":"DAY18","DAY19":"DAY19","DAY20":"DAY20","DAY21":"DAY21","DAY22":"DAY22","DAY23":"DAY23","DAY24":"DAY24","DAY25":"DAY25","DAY26":"DAY26","DAY27":"DAY27","DAY28":"DAY28","DAY29":"DAY29","DAY30":"DAY30","DAY31":"DAY31"};
		 $http({
		        method : "POST",
		        url : "ManagerApprovals?Routing=Dept_att_HR&Routing_type=AJEX&Month_Sel="+Month_Sel+"&ATT_MOD_F_HR="+PAYPERIOD+"&ATT_MOD_F_HR_BU="+BuUnit+" ",
 ...
 $scope.Data_2=<%=EmpDOB%>;
    ...
    $scope.emp_job_type= Data.TYPE;
	 ...
	 $scope.exportData=function(){
        ...
        saveAs(blob, "Bu_attendance_report.xls");
    };
...
$(document).ready(function(){
 ...
 var DataFetching="NONE";
 function call(val1){
	  
	 DataFetching="NONE";
	 document.getElementById("Responce_Message_dialog").innerHTML="";
	// alert(val1.id);
	 document.getElementById("oldDate").innerHTML=val1.name;
 ...
 function dataFetching(val1){
	 
	 
	 if(DataFetching=="CALL"){
		 
		 document.getElementById("Responce_Message_dialog").innerHTML="Request Processed already/cloes the window and try again";
		 return false;
	 }
	 
	
	// alert(val1.id+"~Hai~"+val1.name);
	 var IDS=val1.id.concat('~').concat(val1.name);
	 
	 document.getElementById("Responce_Message_dialog").innerHTML="";
	 
	 var IDS1=val1.id.concat('~').concat(val1.name).concat('~1');
	  // alert(IDS +"::"+ document.getElementById(IDS).innerHTML);
	 var SelectedVal=document.getElementById("popSel").value;
	 
	// Code Added For Ajex Calling Responce of Code;;;;
	   document.getElementById("image_scrl_dialog").style.display='';
		var formData = {Routing:"LeaveMgM",Leave_Type:""+SelectedVal+"",from_date:""+val1.name+"",to_date:""+val1.name+"",HalfDay:"no",Hal_date:""+val1.name+"",compoff:"no",comm_date:""+val1.name+"",to_mail:"nodata",cc_mail:"nodata",subject:"LEAVE BY HR",reason:"HR LEAVE ADJUSTMENTS",HR_ATT:"HRATT",HR_ATT_USER:""+val1.id+""};
		try{
		    $.ajax({
		          type: "post",
		          url: "LeaveManagement",
		          ...
		          success: function(responseData, textStatus, jqXHR) {
		             ...
		             try{
		            	 document.getElementById("Responce_Message_dialog").innerHTML=responseData;
		            	 document.getElementById("image_scrl_dialog").style.display='none';
		            	 
		            	 if((responseData.toString()).indexOf("Your leave successfully processed")!=-1){
		            		 
		            		 document.getElementById(IDS).style.color='green';  
		            		 document.getElementById(IDS).innerHTML=SelectedVal
		          ...
		          error: function(jqXHR, textStatus, errorThrown) {
		              // alert(errorThrown);
		               document.getElementById("Responce_Message_dialog").innerHTML=errorThrown;
		               document.getElementById("image_scrl_dialog").style.display='none';
			 ...
			 document.getElementById("Responce_Message_dialog").innerHTML=err;
			 document.getElementById("image_scrl_dialog").style.display='none';
		}   
								  ...
								  <!--  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
								   <span id='Requ_Date_ERROR' style='color:red;' align='right'> Please Submit your approvals before payroll closing. </span> -->
									<div class="panel-headingattn" >
									  ...
									  <div class="col-md-3">
										<span><i class="fa fa-users" ></i>&nbsp;BusinessUnit.Attendance
										...
										<div class="col-md-8" style="margin-top:5px;">
										 ...
										 <select class="select" id="Month_Sel" >
									  	...
									  	PayPeriod &nbsp;<input type='checkbox' value='PAYPERIOD' ID='ATT_PAYPERIOD_HR' name='ATT_PAYPERIOD_HR'>
									  	
									  	BU &nbsp;<select id="Fill_Bu" name="Fill_Bu" class="select" >
									  	         <option  ng-repeat="(key, value) in Login_bu_jason"  value="{{key}}"  ng-selected="<%=CompanySelected %>"> {{value}} </option>
									  	         </select> 
  ...
  <table   border='0' class="scroll-table"  >
    ...
    <tbody>
      ...
      <tr ng-repeat="x in Data_1">
				...
				<td class="text-center" ng-repeat="k in  x.ATTENDANCE"  > 
			
			<span    class="red-tooltip"  id='{{ x.ID }}~{{ k.ATTDATE }}' data-toggle="tooltip" data-placement="left" title='{{k.DAY_MODE}}' style='color:{{k.DAY.split("#")[1]}}' > 
			
			{{k.DAY.split("#")[0]}} 
			
			</span>
			<!-- <a href="##"   id='{{ x.ID }}'  name='{{ k.ATTDATE }}' onclick='call(this);'> <span class="glyphicon glyphicon-edit" id='{{ x.ID }}~{{ k.ATTDATE }}~1' Style="display:{{k.DAY.split('#')[0]=='A' ?' ':'none'}}"  > </span> </a>
			 -->
			 </td>
			 <td>{{ x.Total_work_Days }}</td>
			  <td>{{ x.Total_Weakoff }}</td>
			   <td>{{ x.Working_Days }}</td>
			    <td>{{ x.Total_Absents }}</td>
			...
			<!-- <td class="text-center"  ng-repeat="(key, value) in x " ng-if="$index >= 2  " style='color:{{ x.DAY+''+$index}}' > 
			
			<span    class="red-tooltip"  data-toggle="tooltip" data-placement="top" title='hai Data' > {{x.DAY$index)}} 
		...
		<!--  	<td class="text-center"  ng-repeat="(key, value) in x " ng-if="$index >= 2  " style='color:{{ value.split("#")[1] }}' > <span    class="red-tooltip"  data-toggle="tooltip" data-placement="top" title='hai Data' > {{ value.split("#")[0] }}  </span>  </a>  <span class="glyphicon glyphicon-edit" Style=" display:{{value.split('#')[0]=='A' ?' ':'none'}} " > </span> </td> -->
...
margin-left:0px;
   ...
   <div id='dialog11' style='display:none'>
    	...
    	<div id="dialog" title="HR Attendance">
  <p> 
    Selected Date:: <span id='oldDate'> </span> </p> <p>
    <select id='popSel' class="select">
  	               <option value="SL">Sick Leave</option>
					<option value="CL" selected>Casual Leave</option>
					<option value="EL">Earned Leave</option>
					<option value="CF">COMP-OFF</option>
					<option value="OD">On Duty</option>
					<option value="LOP">LOP/NOT COMING</option>
					
				</select>
    <span align='center' id='image_scrl_dialog' style='display:none;'>
   											<img src="assets/images/spinner-blue1.gif" width='15%' height='15%' />
 										</span>
   </p>
   
   <p> <span id='Responce_Message_dialog' style="color:red;">   </span> </p>
